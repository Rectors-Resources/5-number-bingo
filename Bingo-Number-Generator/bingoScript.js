...
const generateBingoNumber = () => {
    ...
    if (pickedNumbers.indexOf(rand) === -1) {
        ...
        currNum.innerHTML = `${letterMatch(pickedNumbers[pickedNumbers.length - 1])}, ${letterMatch(pickedNumbers[pickedNumbers.length - 2])}, ${letterMatch(pickedNumbers[pickedNumbers.length - 3])}, ${letterMatch(pickedNumbers[pickedNumbers.length - 4])}, ${letterMatch(pickedNumbers[pickedNumbers.length - 5])}`
...
button.addEventListener('click', loopy) ;
